/* QCC2 PID Controller Library for EasyC V4 or V5
  * Jason McKinney
  * 07/22/2015

  <-- Requires EasyC Timer 2 to be started before use -->
*/
...
float pidExecute(PID *pid, float error) 
{
    ...
    pid->dT = (GetTimer(2) - pid->timer)/1000; //delta time in seconds
    pid->timer = GetTimer(2);
    Wait(10);
